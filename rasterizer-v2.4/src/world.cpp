...
// -------------------------
// STORAGE (single definition)
// -------------------------
std::unordered_map<ChunkCoord, Chunk, ChunkCoordHash> chunks;
ChunkCoord lastPlayerChunk = {INT_MAX, INT_MAX, INT_MAX};
...
void updateChunks(const Vec3& playerPos)
{
    ...
    // only run when entering new chunk
    if (pc.x == lastPlayerChunk.x &&
        pc.y == lastPlayerChunk.y &&
        pc.z == lastPlayerChunk.z)
        return;

    lastPlayerChunk = pc;

    const int radius = 4;
    const int maxDist = radius + 2;
